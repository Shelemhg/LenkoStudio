...
Also implemented in this repository (static landing page UX, deployed on Cloudflare Pages):
- **Multi-page marketing site:** `index.html`, `adam-features.html`, `adam-pricing.html`, `adam-about.html`
- **Challenge selector personalization:** user picks a core pain point → tailored explanation (anchor: `#challenge-selector`)
- **Interactive product tour:** “How it Works” tabs with purpose-built visuals (anchor: `#how-it-works`)
  - Secure sync pipeline demo (“Simulate Sync”)
  - Neural scan demo (“Run Analysis”)
  - Growth trajectory demo (“See Your Growth”)
- **90-day growth predictor:** lightweight calculator + chart output (anchor: `#growth-predictor`)
- **First-hour velocity teaching demo:** time-series graph with “without/with/compare” toggles and an intervention marker (anchor: `#velocity-demo`)
- **Social proof UX:** testimonial carousel/track (anchor: `#testimonials`)
- **Motion system:** scroll-triggered reveals + 3D/tilt effects used to communicate hierarchy and product tangibility (not decoration)
...
2. **Adam Landing Page** (`adam` repository → `adam.lenkostudio.com` + `lenkostudio.com/adam`)
  ...
  - Purpose: Product landing page with hero, interactive product demos, pricing, and conversion CTAs
  - Tech: Static HTML + custom CSS design system (`css/adam-styles.css`) + vanilla JS (SVG/canvas)
...
**Frontend Styling: Custom CSS design system (current implementation)**
- **Design tokens:** CSS variables for colors/typography/spacing (Interstellar dark theme)
- **Why custom CSS here:** no build step, fast iteration, consistent look across pages
- **Interactive visuals:** SVG (graphs), Canvas (scanner), IntersectionObserver (reveal/motion)
